...
void draw() {
  ...
  for (int i = 0; i < 50; i++) {
    for (int j = 0; j < 50; j++) {
      ...
      beginShape();
      texture(ship_texture);
      vertex(i * 250 / 50.0 - 300, j * 250 / 50.0 + 50, -300, i / 50.0, j / 50.0);
      vertex((i + 1) * 250 / 50.0 - 300, j * 250 / 50.0 + 50, -300, (i + 1) / 50.0, j / 50.0);
      vertex((i + 1) * 250 / 50.0 - 300, (j + 1) * 250 / 50.0 + 50, -300, (i + 1) / 50.0, (j + 1) / 50.0);
      vertex(i * 250 / 50.0 - 300, (j + 1) * 250 / 50.0 + 50, -300, i / 50.0, (j + 1) / 50.0);
      endShape();
    }
  }
